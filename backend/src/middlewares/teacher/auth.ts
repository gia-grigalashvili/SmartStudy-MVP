...
export const teacherAuthenticate = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  const accessSecret = getEnvVariable("TEACHER_JWT_ACCESS_SECRET");
  const refreshSecret = getEnvVariable("TEACHER_JWT_REFRESH_SECRET");
  ...
  try {
    ...
    try {
      ...
      return sendError(req, res, 401, "teacherAuthenticateFailed");
    }
  }
};

export const isTeacherVerified = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const stageToken = req.cookies.teacher_verify_stage;
    ...
    const decoded = jwt.verify(
      stageToken,
      getEnvVariable("TEACHER_STAGE_JWT_SECRET")
